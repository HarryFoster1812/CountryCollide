    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            if (data.type === "Topology") {
                ...
                geojson = topojson.feature(data, obj);
            } else {
                geojson = data;
            }
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            .polygonCapColor((d: Feature) =>
                selected.has(getCountryKey(d)) ? "rgba(255, 204, 0, 0.35)" : "rgba(0,0,0,0)"
            )
            .polygonSideColor(() => "rgba(0,0,0,0)")
            .polygonStrokeColor((d: Feature) =>
                selected.has(getCountryKey(d)) ? "#ffcc00" : "rgba(255,255,255,0.35)"
            )
            .polygonAltitude((d: Feature) => (selected.has(getCountryKey(d)) ? 0.02 : 0.004))
    ...
    useEffect(() => {
        ...
        globeRef.current
            ...
            .polygonAltitude((d: Feature) =>
                selected.has(getCountryKey(d)) ? 0.02 : 0.004
            );
    }, [selected]);

    // --- utils ---
    const getCountryKey = (f: Feature) =>
